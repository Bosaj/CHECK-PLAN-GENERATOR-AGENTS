...
def _parse_security_option(headers: dict[str, str], query_params: dict[str, list[str]], option: Any):
    ...
    for name in opt_fields:
        ...
        if metadata is None or not metadata.scheme:
            continue
        _parse_security_scheme(headers, query_params, metadata, name, getattr(option, name))
...
def _parse_security_scheme(
    ...
    if isinstance(scheme, BaseModel):
        ...
        for name in scheme_fields:
            ...
            _parse_security_scheme_value(headers, query_params, scheme_metadata, metadata, name, value)
    else:
        _parse_security_scheme_value(headers, query_params, scheme_metadata, scheme_metadata, field_name, scheme)
