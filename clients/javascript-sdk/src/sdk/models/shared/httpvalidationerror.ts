...
export function httpValidationErrorToJSON(
  ...
  return JSON.stringify(
    HTTPValidationError$outboundSchema.parse(httpValidationError),
  );
}

export function httpValidationErrorFromJSON(
  jsonString: string,
): SafeParseResult<HTTPValidationError, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => HTTPValidationError$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'HTTPValidationError' from JSON`,
  );
}
